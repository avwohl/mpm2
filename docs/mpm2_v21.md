...
format carries the relocation.  um80 0.3.49 hands `low()` of a
relocatable value to the linker, as MACRO-80 does, and the override is
DRI's text but for V2.1's close: linked at 1CBEH with the externals at
DRI's addresses, the V2.0 build is DRI's `GENSYS.COM` 1CBEH-1E5CH byte for
byte.  The source-built V2.0 GENSYS writes the same `MPM.SYS` and
`SYSTEM.DAT` as DRI's, but for the serial number.  DRI's own
`GENSYS.COM` relocates correctly; see [ldrlwr_bug.md](ldrlwr_bug.md).
...
* Resident system processes were built wrong and left out of every
  ...
  * Against DRI's files, `ABORT.RSP`'s header, image and relocation bits
    ...
    ctl-Z).  The other RSPs match in every byte DRI's DATA and INITIAL
    lists define.
  ...
  * Building them from DRI's sources found four uplm80 defects - a
    ...
    0.3.7 and um80 0.3.49, and all of these sources build from DRI's text
    but for their V2.1 changes.
